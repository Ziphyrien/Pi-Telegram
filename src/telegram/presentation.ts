...
async function sendAttachments(
  ...
  for (const attachment of attachments) {
    try {
      ...
      await context.reply(`❌ 附件发送失败：${attachment.label || "未知附件"}\n${(error as Error).message}`).catch(() => undefined);
    }
    first = false;
  }
}
...
export function formatCronStatus(status: SchedulerStatus): string {
  return [
    `⏰ 定时服务：${status.enabled ? "开启" : "关闭"}`,
    `总任务：${status.totalJobs}`,
    `启用：${status.enabledJobs}`,
    `运行中：${status.runningJobs}`,
    `队列中：${status.queuedJobs}`,
    `最近下次触发：${formatDateTime(status.nextRunAtMs)}`,
  ].join("\n");
}
