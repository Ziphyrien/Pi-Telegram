...
import { buildPromptWithRegisteredTools } from "./tools.js";
import {
...
export function createBot(opts: CreateBotOptions): Bot<BotContext> {
  const { botIndex, config, pool, maxResponseLength } = opts;
  ...
  const runPromptRequest = async (
    ...
    try {
      ...
      const promptMessage = buildPromptWithRegisteredTools(message);
      ...
      if (useStream) {
        ...
        try {
          ...
          // Stop any pending stream timer before final render,
          // otherwise a delayed "生成中..." edit may overwrite final content.
          stream.dispose();
        }
        return;
      }
...
function dedupeLoadedImageGroups(
  ...
  const push = (bucket: LoadedImage[], img: LoadedImage) => {
    ...
    let payload = "";
    if (!fid && img.image) {
      payload = `${img.image.mimeType}|${img.image.data}`;
...
function buildStreamingPreview(text: string, tools: string[], limit: number): string {
  ...
  const suffix = "\n\n⏳ 生成中...";
  const available = Math.max(32, limit - toolBlock.length - suffix.length);

  if (!text) return `${toolBlock}⏳ 思考中...`;
  if (text.length <= available) return `${toolBlock}${text}${suffix}`;

  const tail = text.slice(-available);
  return `${toolBlock}…${tail}${suffix}`;
}
