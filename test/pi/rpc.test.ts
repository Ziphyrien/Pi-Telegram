...
import { PiRpc, type PiRpcClient, type PiRpcClientOptions, type RpcClientEvent } from "../../src/pi/rpc.js";
...
async function waitForStarted(rpc: PiRpc): Promise<void> {
  await rpc.getState();
}
...
describe("PiRpc", () => {
    ...
    const { rpc, clients } = createRpc({
      onClient: (client) => {
        client.onPrompt = async () => {
          client.emitEvent({ type: "message_update", assistantMessageEvent: { type: "text_delta", delta: "hello" } } as unknown as RpcClientEvent);
          client.emitEvent({ type: "tool_execution_start", toolName: "read" } as unknown as RpcClientEvent);
          client.emitEvent({ type: "tool_execution_end", toolName: "read", isError: true } as unknown as RpcClientEvent);
          client.emitEvent({ type: "message_update", assistantMessageEvent: { type: "text_delta", delta: " world" } } as unknown as RpcClientEvent);
          client.emitEvent({ type: "agent_end", messages: [{ stopReason: "end" }] } as unknown as RpcClientEvent);
        };
      },
    });
    const deltas: string[] = [];
    ...
    const { rpc, clients } = createRpc({
      onClient: (client) => {
        client.onPrompt = async (message) => {
          ...
          client.emitEvent({ type: "message_update", assistantMessageEvent: { type: "text_delta", delta: message } } as unknown as RpcClientEvent);
          client.emitEvent({ type: "agent_end", messages: [{ stopReason: "end" }] } as unknown as RpcClientEvent);
        };
      },
    });
  ...
  test("surfaces agent errors with stderr context", async () => {
    const { rpc } = createRpc({
      onClient: (client) => {
        ...
        client.onPrompt = async () => {
          client.emitEvent({ type: "message_update", assistantMessageEvent: { type: "error", error: { message: "stream broke" } } } as unknown as RpcClientEvent);
          client.emitEvent({ type: "agent_end", messages: [{ stopReason: "error", content: [{ text: "final failure" }] }] } as unknown as RpcClientEvent);
        };
      },
    });
  ...
  test("delegates model, state, stats, abort, and kill operations to the client", async () => {
    ...
    await rpc.rpcSetModel("p2", "m2");
    await rpc.rpcSetThinkingLevel("high");
    rpc.abort();
    await new Promise((resolve) => setImmediate(resolve));
  ...
  test("ignores streaming hook failures and still resolves", async () => {
    const { rpc } = createRpc({
      onClient: (client) => {
        client.onPrompt = async () => {
          client.emitEvent({ type: "tool_execution_start", toolName: "shell" } as unknown as RpcClientEvent);
          client.emitEvent({ type: "tool_execution_end", toolName: "shell", isError: true } as unknown as RpcClientEvent);
          client.emitEvent({ type: "message_update", assistantMessageEvent: { type: "text_delta", delta: "ok" } } as unknown as RpcClientEvent);
          client.emitEvent({ type: "agent_end", messages: [{ stopReason: "end" }] } as unknown as RpcClientEvent);
        };
      },
    });
  ...
  test("rejects aborted agent_end responses", async () => {
    const { rpc } = createRpc({
      onClient: (client) => {
        client.onPrompt = async () => {
          client.emitEvent({ type: "agent_end", messages: [{ stopReason: "aborted" }] } as unknown as RpcClientEvent);
        };
      },
    });
